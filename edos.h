...
extern double dspin0dt (double t, double *y) 
{
    return
     ...
     (
      f1(y[1]) * 0.5 * (1.f + pow(cos(y[5]), 2)) * y[4] / ni(y[0]) - \
      f2(y[1]) * cos(y[5])
     );
}
...
extern double dspin1dt (double t, double *y) 
{
    return
     ...
     (
      f1(y[1]) * 0.5 * (1.f + pow(cos(y[3]), 2)) * y[2] / ni(y[0]) - \
      f2(y[1]) * cos(y[4])
     );
}
...
// {
//     return 2.f / (m1prime * pow(B1_ptr->a, 7)) + auxdadt(B0_ptr) + auxdadt(B1_ptr);
// }

// extern double de2dt (double t, double *y) 
// {
//     return 9.f * B1_ptr->a / (m1prime * pow(B1_ptr->a, 8)) + auxdedt(B0_ptr) + auxdedt(B1_ptr);
// }
